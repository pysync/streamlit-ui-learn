...
def insert_and_index_artifact(doc_id: str, title: str, content: str):
    # Create and insert the artifact
    create_new_artifact(doc_id, title, content)
...
def test_list_artifacts():
    create_new_artifact("doc3", "Title 3", "Content 3")
    create_new_artifact("doc4", "Title 4", "Content 4")
    artifacts, total = list_artifacts(limit=10, page=1)
    assert len(artifacts) >= 2

# Test case to check if artifact is indexed after it's created
def test_reindex_all_artifacts():
    data = [{
        "doc_id": f"doc{i}",
        "title": f"Title {i}",
        "content": f"Content {i}"
    ...
    for item in data:
        create_new_artifact(item["doc_id"], item["title"], item["content"])
    
    artifacts, total = list_artifacts(limit=-1)
